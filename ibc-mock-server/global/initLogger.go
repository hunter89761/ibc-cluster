...
import (
	...
	"github.com/ibc-mock-server/global/file"
...
func LoggerSetup() {
	cfgLog := config.GetCfg().Logger
	options := Options{
		SavePath:     cfgLog.SavePath,
		EncoderType:  cfgLog.EncoderType,
		EncodeLevel:  cfgLog.EncodeLevel,
		EncodeCaller: cfgLog.EncodeCaller,
	}
